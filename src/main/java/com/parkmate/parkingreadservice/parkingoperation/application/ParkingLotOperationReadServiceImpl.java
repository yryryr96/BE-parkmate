...
public class ParkingLotOperationReadServiceImpl implements ParkingLotOperationReadService {
    ...
    public Set<String> validateOperationByUuidAndDateRange(List<String> parkingLotUuids,
        ...
        return getAvailableParkingLotUuid(requestStart, requestEnd, operations);
    }

    private Set<String> getAvailableParkingLotUuid(LocalDateTime requestStart,
                                                   LocalDateTime requestEnd,
                                                   List<ParkingLotOperationRead> operations) {
        /**
    ...
    private boolean isEntirePeriodValid(List<ParkingLotOperationRead> operations,
                                        LocalDateTime requestStart,
                                        LocalDateTime requestEnd) {
        ...
        if (operations.size() != expectedDays) {
            return false;
        }

        // 2. 예약 기간이 하루인 경우 (시작일과 종료일이 같음)
        if (operations.size() == 1) {
            ParkingLotOperationRead op = operations.get(0);
            // 요청 시작 시간이 운영 시작 시간보다 전이 아니고,
            // 요청 종료 시간이 운영 종료 시간보다 후가 아니면 통과.
            return isRequestWithinOperation(requestStart, requestEnd, op);
        }
        // 3. 예약 기간이 여러 날인 경우
        else {

            return isFirstDayValid(requestStart, operations.get(0)) &&
                   isEndDayValid(requestEnd, operations.get(operations.size() - 1)) &&
                   areIntermediateDayValid(operations);
        }
    }
    ...
    private boolean isFirstDayValid(LocalDateTime requestStart,
                                    ParkingLotOperationRead firstDayOp) {

        LocalDateTime firstDayLimit = LocalDateTime.of(firstDayOp.getEndDateTime().toLocalDate(), LocalTime.of(23, 59, 59));
        return requestStart.isBefore(firstDayOp.getStartDateTime()) || firstDayOp.getEndDateTime().isBefore(firstDayLimit);
    }

    private boolean isEndDayValid(LocalDateTime requestEnd,
                                  ParkingLotOperationRead lastDayOp) {
        return !lastDayOp.getStartDateTime().equals(lastDayOp.getEndDateTime().toLocalDate().atStartOfDay()) ||
               requestEnd.isAfter(lastDayOp.getEndDateTime());
    }

    private boolean areIntermediateDayValid(List<ParkingLotOperationRead> operations) {

        for (int i = 1; i < operations.size() - 1; i++) {
            ParkingLotOperationRead intermediateDayOp = operations.get(i);
